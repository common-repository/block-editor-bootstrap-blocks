...
/*
	...
	Plugin URI:			https://wp-speedup.eu/shop/wordpress-plugins/pro-plugins/block-editor-bootstrap-blocks-pro/
...
class BootstrapBlocks{
	...
	function maybe_inject_inline_css( $content ){
		if( $this->is_valid_buffer( $content ) ){
			...
			if( $this->has_PRO_version ){
				$spacing_css .= BootstrapBlocksPRO::instance()->get_extra_css();
			}
	...
	public function init(){
		...
		$active_plugins = array_merge( get_option( 'active_plugins', [] ), array_keys( $active_sitewide_plugins ) );
		if( in_array( 'block-editor-bootstrap-blocks-pro/block-editor-bootstrap-blocks-pro.php', $active_plugins ) ){
			$this->has_PRO_version = true;
		}

		if( ! $this->has_PRO_version ){
			add_action( 'admin_notices', array( $this, 'admin_notices' ) );
			add_action( 'wp_ajax_bootstrap_notice_dismissed', array( $this, 'bootstrap_notice_dismissed' ) );
		}
	...
	function admin_notices(){
		if( ! get_user_meta( get_current_user_id(), 'bootstrap_notice_dismissed' ) ){
			if( function_exists('get_current_screen') && isset( get_current_screen()->id ) && in_array( get_current_screen()->id, array( 'dashboard', 'plugins', 'plugin-install' ) ) ){
				$this->pro_version_promotion( 1 );
			}
		}
	}

	function pro_version_promotion( $is_dismissible = false ){ ?>
		<div class="bootstrap-notice notice notice-alt notice-large notice-success <?php echo $is_dismissible ? 'is-dismissible' : '' ?>">
			<h1>Block Editor Bootstrap Blocks</h1>
			<big style="line-height:1.6">
				<strong><?php _e( 'Unlock premium features with our PRO version!', 'bootstrap' ) ?></strong><br>
				<?php _e( 'Get it now and recieve all the cool upcomming features for free later!', 'bootstrap' ) ?><br>
				<?php _e( 'Introductory pricing will not last! Every update can raise the price a little bit.', 'bootstrap' ) ?>
			</big>
			<hr style="opacity:0">
			<a href="https://wp-speedup.eu/shop/wordpress-plugins/pro-plugins/block-editor-bootstrap-blocks-pro/" target="_blank" class="button button-primary">Read more</a>
			<div style="height:10px"></div>
		</div><?php
		if( $is_dismissible ){ ?>
			<script>
			jQuery(document).ready(function($){
				$(document).on( 'click', '.bootstrap-notice .notice-dismiss', function(){
					$.post( ajaxurl, { action: 'bootstrap_notice_dismissed', _wpnonce: '<?php echo wp_create_nonce('bootstrap_notice_dismissed') ?>' });
				});
			});
			</script><?php
		}
	...
	function bootstrap_notice_dismissed(){
		if( defined('DOING_AJAX') && DOING_AJAX && check_ajax_referer('bootstrap_notice_dismissed') ){
			add_user_meta( get_current_user_id(), 'bootstrap_notice_dismissed', 1, true );
		}
	}
	...
	function fix_attributes_for_ServerSideRender(){
		...
		foreach( $registered_blocks as $block ){
			...
			$responsiveObjects = [ 'bsSnapping', 'bsDisplay', 'bsAlignment', 'bsFlexDirection', 'bsFlexWrap', 'bsFlexJustifyContent', 'bsFlexAlignItems', 'bsFlexAlignContent', 'bsFlexGap', 'bsFlexGrow', 'bsFlexShrink', 'bsFlexBasis', 'bsFlexAlignSelf', 'bsFlexOrder', 'bsPosition', 'bsPositionZindex', 'bsPositionTop', 'bsPositionRight', 'bsPositionBottom', 'bsPositionLeft', 'bsWidth', 'bsMinWidth', 'bsMaxWidth', 'bsHeight', 'bsMinHeight', 'bsMaxHeight', 'bsAspectRatio' ];
	...
	function render_block( $block_content, $block ){
		...
		if( isset( $block['attrs'] ) && isset( $block['attrs']['tempID'] ) && $block['attrs']['tempID'] ){
			if( $this->has_PRO_version ){
				$extraCSSitems = [ 'bsFlexGap', 'bsFlexBasis', 'bsPosition', 'bsPositionZindex', 'bsPositionTop', 'bsPositionRight', 'bsPositionBottom', 'bsPositionLeft', 'bsWidth', 'bsMinWidth', 'bsMaxWidth', 'bsHeight', 'bsMinHeight', 'bsMaxHeight', 'bsAspectRatio' ];
			}else{
				$extraCSSitems = [ 'bsSpacing', 'bsFlexGap', 'bsFlexBasis', 'bsPosition', 'bsPositionZindex', 'bsPositionTop', 'bsPositionRight', 'bsPositionBottom', 'bsPositionLeft', 'bsWidth', 'bsMinWidth', 'bsMaxWidth', 'bsHeight', 'bsMinHeight', 'bsMaxHeight', 'bsAspectRatio' ];
			}
			...
			foreach( $this->final_css as $block ){
				if( ! isset( $alreadyAdded[ $block['tempID'] ] ) ){
					...
					foreach( $breakpoints as $breakpointSize => $breakpointData ){
						...
						if( ! $this->has_PRO_version && isset( $block['bsSpacing'] ) ){
						...
						$basicProperties = [
							...
							'bsWidth' => [ 'property' => 'width', 'convertToPixels' => true ],
							'bsMinWidth' => [ 'property' => 'min-width', 'convertToPixels' => true ],
							'bsMaxWidth' => [ 'property' => 'max-width', 'convertToPixels' => true ],
							'bsHeight' => [ 'property' => 'height', 'convertToPixels' => true ],
							'bsMinHeight' => [ 'property' => 'min-height', 'convertToPixels' => true ],
							'bsMaxHeight' => [ 'property' => 'max-height', 'convertToPixels' => true ],
							'bsAspectRatio' => [ 'property' => 'aspect-ratio', 'convertToPixels' => false ],
	...
	function load_settings(){
		...
		$this->settings = array_merge(
			...
			apply_filters( 'extra_BootstrapBlocks_settings_array', [] ),
			$this->settings,
		);
	...
	function enqueue_block_editor_assets(){
		...
		$build_path = 'build';
		if( $this->has_PRO_version ){
			$build_path = '../block-editor-bootstrap-blocks-pro/build';
		}

		wp_enqueue_style(
			'bootstrap-blocks-editor',
			plugins_url( $build_path . '/index.css', __FILE__ ),
			array(),
			filemtime( plugin_dir_path( __FILE__ ) . '/' . $build_path . '/index.css' )
		);

		wp_enqueue_script(
			'bootstrap-blocks',
			plugins_url( $build_path . '/index.js', __FILE__ ),
			$dependencies,
			filemtime( plugin_dir_path( __FILE__ ) . '/' . $build_path . '/index.js' )
		);
		...
		wp_localize_script(
			...
			array(
				...
				'useTooltipPlacement' => is_wp_version_compatible('6.4'),
			)
		);
	}
	...
	function filter_plugin_actions( $links, $file ){
		...
		if( ! $this->has_PRO_version ){
			array_unshift( $links, '<a href="https://wp-speedup.eu/shop/wordpress-plugins/pro-plugins/block-editor-bootstrap-blocks-pro/" target="_blank" style="font-weight:bold;color:#f00">' . esc_html__( 'PRO version', 'dubeb' ) . '</a>' );
		}
		return $links;
	}
	...
	function admin_options_page(){
		...
		$this->tab = isset( $_GET['tab'] ) ? sanitize_key( $_GET['tab'] ) : 'general';
	...
	function tab_general_options(){ ?>
		<table class="form-table">
			<tbody>
				...
				<?php do_action( 'extra_BootstrapBlocks_settings', $this->settings ) ?>
	...
	function wp_enqueue_scripts_override(){
		...
		wp_enqueue_style( 'bootstrap-blocks', plugins_url( $build_path . '/style-index.css', __FILE__ ), array(), filemtime( plugin_dir_path( __FILE__ ) . $build_path . '/style-index.css' ) );
	}
}
